...
%% Main function which perform Bayesion Classification based on given Case and returns Target and Non Target Scores
...
% 
r = 1*min(data(:,1)):.5:1*max(data(:,1)); %// x axis
r1 = 1*min(data(:,2)):.5:1*max(data(:,2)); %// y axis
...
print('-djpeg', ['NLS_Gaussian_PDF_Case_',num2str(caseno),'.jpg'], '-r300');
...
set(gca,'FontSize',14,'FontWeight','bold')
print('-djpeg', ['NLS_Contours_Case_',num2str(caseno),'.jpg'], '-r300');
hold off
...
set(gca,'FontSize',14,'FontWeight','bold')
print('-djpeg', ['NLS_Decision_Boundary_Case_',num2str(caseno),'.jpg'], '-r300');
