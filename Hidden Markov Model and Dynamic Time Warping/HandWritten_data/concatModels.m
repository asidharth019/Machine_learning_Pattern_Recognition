function [  ] = concatModels(  )
%Concates HMM Model Files


mod1=textread('ai.txt.hmm','%s');
mod2=textread('ba.txt.hmm','%s');
mod3=textread('la.txt.hmm','%s');
    ...
    for j= 1:3
        for f = 1:3
            ...
            f1(size(f1,1),1)=0.500000;
             ...
             f2(size(f2,1),:)=f3(1,:);
            f2(size(f2,1),1)=0.500000;
             f2(size(f2,1)-1,1)=0.500000;
            
            f3(size(f3,1),1)=0.000000;
            
            dummy1=f2(1,1:size(f2,2)); %double(zeros(1,size(f1,2))); 
            dummy1(1,1)=1.000000;  
                        
            dummy2=f3(1,1:size(f3,2)); %double(zeros(1,size(f1,2))); 
            dummy2(1,1)=1.000000;
            dummy = f3(size(f3,1),:);
            dummy(1,1) = 0.000000;
             dummy=double(zeros(1,size(f1,2)));
            
%                final=[f1;dummy;dummy1;f2;dummy;dummy2;f3];
                final=[f1;f2;f3];
            
            file4=fopen(['hmm-1.04/tmp/' num2str(i) num2str(j) num2str(f) '.txt'],'w');
